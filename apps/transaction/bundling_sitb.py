...
from apps.transaction.data import data_icd_x_code, data_encounter_class, data_icd_x_code, \
    data_condition_clinical_status, data_questionnaire_response_type_diagnosis_sitb, \
    data_questionnaire_response_classification_location_anatomy_sitb, \
    data_questionnaire_response_classification_treatment_history_sitb, \
    data_questionnaire_response_end_result_of_treatment_tb_sitb, data_type_observation_bta, \
    data_type_observation_thorax, data_medication_kfa
    ...
    if transaction_sitb.raw_data is not None and count_retry <= 7:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                response_data = response_data_raw if isinstance(response_data_raw, dict) else settings.MESSAGE_ERRORS

                # jika status == berhasil / update berhasil
                if isinstance(response_data, dict) and response_data.get('status') in [
                    settings.STATUS_RESPONSE_SITB['BERHASIL'], settings.STATUS_RESPONSE_SITB['UPDATE_BERHASIL']]:
...
def patient(data):
    ...
    try:
        ...
        if data["province"] is not None or data["province"] != "":
            get_province = Province.objects.filter(code=data["province"]).first()
            if get_province:
                patient_province = data["province"]
    except KeyError:
        pass

    try:
        # validate patient kd_kabupaten_pasien not null
        if data["district"] is not None or data["district"] != "":
            get_district = District.objects.filter(code=data["district"]).first()
            if get_district:
                patient_district = data["district"]
    except KeyError:
        pass

    try:
        if data["address"] is not None or data["address"] != "":
    ...
    data_patient = {
        ...
        'alamat_lengkap': patient_address,
        'id_propinsi_pasien': patient_province,
        'kd_kabupaten_pasien': patient_district,
    }
...
def questionnaire_response(data):
    ...
    questionnaire_response = {
        "tipe_diagnosis": data_questionnaire_response_type_diagnosis_sitb([type_diagnosis])[
            0] if type_diagnosis else "",
        "klasifikasi_lokasi_anatomi":
            data_questionnaire_response_classification_location_anatomy_sitb([location_anatomy])[
                0] if location_anatomy else "",
        "klasifikasi_riwayat_pengobatan":
            data_questionnaire_response_classification_treatment_history_sitb([treatment_history])[
                0] if treatment_history else "",
        "hasil_akhir_pengobatan": data_questionnaire_response_end_result_of_treatment_tb_sitb([end_result_treatment])[
            0] if end_result_treatment else "",
    }
...
def episode_of_care(data):
    ...
    start_date_formated = datetime.strptime(period_start, settings.DATE_TIME_FORMAT_TIMEZONE).strftime("%Y%m%d")

    if period_end:
        end_date_formated = datetime.strptime(period_end, settings.DATE_TIME_FORMAT_TIMEZONE).strftime("%Y%m%d")
    else:
        end_date_formated = ""
