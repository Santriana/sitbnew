...
def generate_random_code_transaction_id():
    ...
    return ''.join(random.choice(string_name + string_digit) for i in range(3))
...
def transaction_request_update(episode_of_care_ihs_id, request_type):
    data = {
        ...
        "request": {
            "method": "PUT",
            "url": "{}/{}".format(request_type, episode_of_care_ihs_id)
        }
    }
    return data
...
def encounter(encounter_uuid, organization_id,
    ...
    data_default = {
        ...
        "identifier": [
            {
                "system": "http://sys-ids.kemkes.go.id/encounter/{}".format(organization_id),
        ...
        "subject": {
            "reference": "{}/{}".format(settings.PATIENT_RESOURCE, patient_id),
        ...
        "participant": [
            {
                ...
                "individual": {
                    "reference": "{}/{}".format(settings.PRACTITIONER_RESOURCE, practitioner_id),
        ...
        "serviceProvider": {
            "reference": "{}/{}".format(settings.ORGANIZATION_RESOURCE, service_provider)
        },
        "episodeOfCare": [
            {
...
def episode_of_care(episode_of_care_uuid,
    ...
    data_default = {
        ...
        "managingOrganization": {
            "reference": "{}/{}".format(settings.ORGANIZATION_RESOURCE, organization_id)
        },
        "careManager": {
            "reference": "{}/{}".format(settings.PRACTITIONER_RESOURCE, practitioner_id),
            "display": "{}".format(practitioner_name)
...
def medication_request(medication_uuid, medication_request_uuid,
    ...
    data_default = {
        "resource": {
            ...
            "identifier": [{
                "system": "http://sys-ids.kemkes.go.id/prescription/{}".format(organization_id),
                "use": "official",
    ...
    if data:
        for result in data:
            ...
            data_default = {
                ...
                "identifier": [{
                    "system": "http://sys-ids.kemkes.go.id/medication/{}".format(organization_id),
                    "use": "official",
                    "value": "{}".format(uuid.uuid4())
                }],
                "form": {
...
def diagnostic_report(
    ...
    data_default = {
        ...
        "identifier": [
            {
                "system": "http://sys-ids.kemkes.go.id/diagnostic/{}/lab".format(organization_id),
                "use": "official",
